...
func (this *ServerProtocol) PackAction(Output []byte) (err error) {
	...
	if len(this.identify) == 0 {
		err = Log.RetError("[server] identify invalid, routepoint: %v, mainid: %v, subid: %v.", this.routepoint, this.mainid, this.subid)
		return
	}

	if len(this.remoteAddr) == 0 {
		err = Log.RetError("[server] remoteAddr invalid, routepoint: %v, mainid: %v, subid: %v.", this.routepoint, this.mainid, this.subid)
		return
	}
...
func (this *ServerProtocol) SetCmd(routepoint, mainid, subid uint16, data []byte) {
	...
	Log.FmtPrintf("[server] SetCmd routepoint: %v, mainid: %v, subid: %v, data len: %v.", routepoint, mainid, subid, this.length)
}

func (this *ServerProtocol) PackMsg(routepoint, mainid, subid uint16, msg proto.Message) (out []byte, err error) {
	data, err := proto.Marshal(msg)
	if err != nil {
...
func (this *ServerProtocol) PackMsg4Client(routepoint, mainid, subid uint16, msg proto.Message) (out []byte, err error) {
	data, err := proto.Marshal(msg)
	if err != nil {
...
func (this *ServerProtocol) UnPackMsg4Client(InData []byte) (pos int, err error) {
	...
	this.srcdata = InData[:]
	Log.FmtPrintf("message head: routepoint: %v, mainid: %v, subid: %v, srcdata len: %v.", this.routepoint, this.mainid, this.subid, len(this.srcdata))
	return pos, nil
}
...
func (this *ServerProtocol) UnPackMsg4Svr(InData []byte) (pos int, err error) {
	...
	Log.FmtPrintln("server UnPackMsg4Client-> len: ", this.length)
	this.length = binary.LittleEndian.Uint32(InData[pos:])
	pos += 4
