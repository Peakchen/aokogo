...
func (self *TSvrBalanceV1) NewBalance(){

}

func (self *TSvrBalanceV1) AddSvr(svr string){
	_, ok := self.sb[svr]
	if ok {
		return
	}
...
func (self *TSvrBalanceV1) Push(svr string) {
	ex, ok := self.sb[svr]
	if ok {
		return
	}
...
func (self *TSvrBalanceV1) GetSvr() (s string) {
	var (
		secmin int32 = 0
		max int32 = 0
		loop int = 0
